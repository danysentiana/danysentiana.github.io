...
const GitHubCalendar = ({ username }: { username: string }) => {
    ...
    // Fetch contribution data — try multiple CORS proxies with fallback
    const fetchContributions = useCallback(async () => {
        ...
        for (const proxyUrl of proxyUrls) {
            try {
                ...
                if (!svgText.includes("data-date")) continue;
        ...
        // All proxies failed
        console.warn("All CORS proxies failed for GitHub calendar");
        setLoading(false);
    }, [username]);
    ...
    const getColor = (count: number): string => {
        ...
        if (resolvedTheme === "dark") {
            const colors: Record<number, string> = { 1: "#854d0e", 2: "#a16207", 3: "#ca8a04", 4: "#facc15" };
            return colors[Math.min(count, 4)] || "#facc15";
        } else {
            const colors: Record<number, string> = { 1: "#bfdbfe", 2: "#93c5fd", 3: "#3b82f6", 4: "#1d4ed8" };
    ...
    // Use img fallback when proxies fail (no contribution data)
    const useImgFallback = !loading && contributions.size === 0;
        ...
        >
            <div className="bg-neutral-100 dark:bg-neutral-900 rounded-xl border p-4 md:p-6 shadow-sm overflow-x-auto">
                ...
                <div className="relative min-w-[700px]">
                    {useImgFallback ? (
                        /* Fallback: img tag when proxies fail */
                        <div className="flex justify-center">
                            <img
                                src={`https://ghchart.rshah.org/${resolvedTheme === "dark" ? username : `3b82f6/${username}`}`}
                                alt="GitHub contribution chart"
                                className="w-full max-w-[700px]"
                                style={resolvedTheme === "dark" ? { filter: "invert(1) brightness(0.85)" } : undefined}
                            />
                        </div>
                    ) : (
                        /* Custom grid with tooltips */
                        <>
                            {/* Tooltip */}
                            {tooltip && (
                                <div
